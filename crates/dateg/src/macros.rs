...
macro_rules! execute {
    ...
    (@ $eg:expr; = $value:ident ($table_token:ident $($args:tt)*)) => {
        #[cfg(false)] fn $table_token() {} // syntax highlighting hack
        let args = ($($crate::execute!(@@@ = $eg; $args),)*);
        let $value = $eg.row_add($table_token, args);
    };
    // Support for nested syntax
    (@@@ = $eg:expr; $arg:ident) => { $arg };
    (@@@ = $eg:expr; ($table:ident $($args:tt)*)) => {{
        $crate::execute!(@ $eg; = tmp ($table $($args)*));
        tmp
    }};
